...
(function () {
  ...
  var ICONS = {
    ...
    timer:
      '<line x1="10" x2="14" y1="2" y2="2"/><path d="M12 14v-4"/><circle cx="12" cy="14" r="8"/>',
    wallet:
  ...
  (function buildCards() {
    ...
    if (tt) {
      tt.innerHTML = TIDE.map(function (c) {
        return (
          '<div class="tide__card">' + svg(c[1]) + "<b>" + c[0] + "</b></div>"
        );
      }).join("");
    }
    ...
    if (gt) {
      gt.innerHTML = GALLERY.map(function (c) {
        ...
        return (
          '<figure class="gallery__card"><img loading="lazy" decoding="async" src="' +
          src +
          '" alt="PraxisOS ' +
          c[0] +
          ' screen" /><figcaption>' +
  ...
  var animReady =
    root.classList.contains("anim") && gsap && ScrollTrigger && THREE;
  if (root.classList.contains("anim") && !animReady)
  ...
  var CSSVAR = (function () {
    ...
    var get = function (n, fallback) {
      return (cs.getPropertyValue(n) || "").trim() || fallback;
    };
    return {
      gold: get("--gold", "#d8a75b"),
      goldRgb: get("--gold-rgb", "216, 167, 91"),
  ...
  var FRAG = [
    ...
    "varying vec2 vUv;",
    "uniform float u_time,u_aspect,u_blob_radius,u_blob_edge,u_blob_rot,u_opacity,u_flood,u_calm,u_clipTop,u_trailAmt;",
    "uniform vec2 u_trail[5];",
    ...
    "  float trail = 0.0;",
    // Each point lags the one before it, so the chain reads as a wake. Measured
    // in screen space: the hero leaves the body unstretched, and the closing
    // section is flooded, so shape distortion there does not matter.
    "  for (int i = 0; i < 5; i++) {",
    "    float reach = 0.34 - float(i) * 0.045;",
    "    float w = 1.0 - smoothstep(0.0, max(reach, 0.05), length(p - u_trail[i]));",
    "    trail = max(trail, w * (1.0 - float(i) * 0.16));",
    "  }",
    "  trail *= u_trailAmt;",
    "  float sdf = len - rr - trail * 0.17;",
    ...
    "  col *= 0.88 + 0.12 * smoothstep(-0.7, 0.7, shade);",
    "  float depth = smoothstep(0.0, max(rr*0.85, 0.001), rr - len);",
    "  col *= 0.66 + 0.44 * depth;",
    "  col += u_colorB * rim * 0.3;",
    // Also lights the surface, so the wake still reads once the body floods.
    "  col += u_colorB * trail * 0.3;",
  ...
  var uniforms = {
    ...
    u_clipTop: { value: 2 },
    u_trailAmt: { value: 0 },
    u_trail: { value: [new THREE.Vector2(9, 9), new THREE.Vector2(9, 9), new THREE.Vector2(9, 9),
      new THREE.Vector2(9, 9), new THREE.Vector2(9, 9)] },
  ...
  var SMOOTH = 7;
  var ctaHot = false;   // is the closing section on screen?
  // Pointer trail. Kept off-screen until the pointer is first seen so no wake
  // appears before the reader has moved anything.
  var pointer = { x: -9999, y: -9999, seen: false };
  window.addEventListener("pointermove", function (e) {
    pointer.x = e.clientX; pointer.y = e.clientY;
    if (!pointer.seen) {
      pointer.seen = true;
      var sx = (e.clientX / window.innerWidth * 2 - 1) * uniforms.u_aspect.value;
      var sy = 1 - 2 * (e.clientY / window.innerHeight);
      for (var i = 0; i < 5; i++) uniforms.u_trail.value[i].set(sx, sy);
    }
  }, { passive: true });

  function updateTrail(k) {
    var pts = uniforms.u_trail.value;
    if (pointer.seen) {
      var tx = (pointer.x / window.innerWidth * 2 - 1) * uniforms.u_aspect.value;
      var ty = 1 - 2 * (pointer.y / window.innerHeight);
      pts[0].x += (tx - pts[0].x) * Math.min(1, k * 3.2);
      pts[0].y += (ty - pts[0].y) * Math.min(1, k * 3.2);
      for (var i = 1; i < pts.length; i++) {
        var f = Math.min(1, k * (2.6 - i * 0.42));
        pts[i].x += (pts[i - 1].x - pts[i].x) * f;
        pts[i].y += (pts[i - 1].y - pts[i].y) * f;
      }
    }
    uniforms.u_trailAmt.value += (target.trail - uniforms.u_trailAmt.value) * k;
  }
  ...
  gsap.ticker.add(function (time, deltaMS) {
    ...
    if (!heroFluid()) target.trail = ctaHot ? 1 : 0;
    var dx = target.cx - cen.x,
      dy = target.cy - cen.y;
    if (target.lock) {
      cen.x = target.cx;
      cen.y = target.cy;
  ...
  if (liteFluid) {
    ...
    ScrollTrigger.create({
      ...
      onToggle: function (self) { ctaHot = self.isActive; },
  ...
  function measureModules() {
    ...
    colFx = [0, 1, 2].map(function (c) {
      return (gridLeft + ((c + 0.5) * gridW) / 3) / vw;
    });
  }
  function moduleReact(p) {
    ...
    for (var i = 0; i < moduleCards.length; i++) {
      ...
      if (passed > 0.999 && bell < 0.02) {
        ...
        card.style.transform =
          "translateY(" + ((1 - passed) * 20 - bell * 6).toFixed(1) + "px)";
  ...
  ScrollTrigger.create({
    ...
    onUpdate: function (self) {
      ...
      target.cx = lerp(OFF_L, OFF_R, p);
      target.cy = 0; // through the centred grid
      target.sx = 2.6;
      target.sy = 0.78;
      target.radius = 0.5;
      target.edge = 0.03;
  ...
  var tideScatter = (function () {
    ...
    for (var m = 0; m < n; m++) {
      ...
      if (m && Math.abs(lanes[m] - lanes[m - 1]) < 0.45)
        lanes[m] += lanes[m] > lanes[m - 1] ? 0.45 : -0.45;
      out.push({
  ...
  ScrollTrigger.create({
    ...
    onToggle: function (self) {
      tideTrack.style.visibility = self.isActive ? "visible" : "hidden";
    },
    onUpdate: function (self) {
      var p = self.progress;
      target.lock = false;
      target.cx = lerp(OFF_R, OFF_L, p);
      target.cy = 0;
      target.sx = 3.0;
      target.sy = 1.25;
      target.radius = 0.7;
      ...
      var n = tideCards.length;
      var spreadY = Math.min(230, vh * 0.3);
      for (var i = 0; i < n; i++) {
        var s = tideScatter[i];
  ...
  ScrollTrigger.create({
    ...
    onUpdate: function (self) {
      ...
      if (menuStage)
        menuStage.style.opacity = smoothstep(0.16, 0.42, p).toFixed(3);
      if (dragHint)
        dragHint.style.opacity = smoothstep(0.44, 0.62, p).toFixed(3);
    },
  });
    ...
    .timeline({
      scrollTrigger: {
        ...
        onUpdate: function (self) {
          ...
          target.lock = false;
          target.rot = 0;
          target.cx = lerp(OFF_L, 0, smoothstep(0, 0.6, p));
          target.cy = 0;
          target.sx = 1.8;
          target.sy = 1.6;
          target.calm = 1;
          ...
          target.edge = 0.05;
          target.flood = smoothstep(0.25, 1, p);
          target.opacity = smoothstep(0, 0.15, p);
        },
      },
    })
    .fromTo(
      ".cta__inner, .cta footer",
      { opacity: 0, y: 20 },
